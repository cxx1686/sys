...
" ), htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	...
	function get_field($table, $field, $id){	//获取单个值
	...
	function get_count($sql){
		$sel = "select count(1) as c from ";
		$sql = explode("from",$sql);
		$sql = end($sql);
		$sql = explode("limit",$sql);
		$sql = $sql[0];
		$sql = $sel.$sql." limit 0,1";
		$v = $this->get_row($sql);
		return $v["c"];
	}
	 ...
	 * @param array 	$dataArray	字段数组
	 * @return int 		id	记录ID
	 */
	function update($table, $dataArray, $where, $id = 0){
		if($id>0){
			if(!$where) $where = "where id=$id";
			$value="";
